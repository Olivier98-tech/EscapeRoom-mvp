# ✅ Voorbereidingschecklist — Escape Room 2150

Print deze pagina en vink af tijdens het klaarzetten.

## 📦 Een week van tevoren

- [ ] Test de volledige escape room zelf van begin tot eind (ca. 30 min als je de antwoorden kent — zie FACILITATOR-GUIDE.md)
- [ ] Kies: online spelen via GitHub Pages, of lokaal (map openen en `index.html` starten)
- [ ] Regel per team één laptop of tablet **met werkende speakers**
- [ ] Regel per team minimaal één telefoon met QR-scanner (voor level 1 en 6)
- [ ] (Optioneel) Fysieke aanwijzing voor **level 4** — niet meer nodig, de puzzel is op het scherm oplosbaar (zie hieronder)

## 🖨 Fysieke aanwijzing level 4 (optioneel)

Level 4 is nu **volledig op het scherm** oplosbaar: de log geeft vier logica-
aanwijzingen waaruit spelers de volgorde **◉ ⚪ ▲ ⚫** afleiden. Printen hoeft dus
niet meer.

Wil je er tóch een fysieke zoektocht van maken? Verstop dan een briefje met
**◉ ⚪ ▲ ⚫** in de ruimte en laat de on-screen log weg (of gebruik hem als
extra hint).

## 🗓 Op de dag zelf — per apparaat

- [ ] Open de escape room in de browser en zet het scherm op volledig (F11)
- [ ] Controleer het geluid (klik ergens en test het 🔊-icoon)
- [ ] Open `admin.html` → log in (PIN staat in de spelleider-gids) → **Reset alles**
- [ ] Stel de gewenste speelduur in (standaard 60 min)
- [ ] Sluit het admin-tabblad — laat alleen de intropagina open staan
- [ ] (Alleen als je de fysieke variant gebruikt) Controleer dat het level 4-briefje verstopt ligt

## 🎬 Briefing voor de spelers (voorlezen)

> „Welkom, cyberagenten. Het jaar is 2150. Een kwaadaardige AI heeft de wereld
> vergrendeld en jullie zijn de laatste hoop van de mensheid. Achter dit scherm
> liggen zes beveiligingslagen. Kraak ze allemaal en activeer de zelfvernietiging
> van de AI — vóór de tijd om is. Let op: foute antwoorden blokkeren het systeem
> tijdelijk, dus overleg goed voordat je iets invoert. Kom je er niet uit? Na twee
> foute pogingen verschijnt er een hint-knop. Eén aanwijzing bevindt zich niet op
> het scherm, maar ergens in deze ruimte... Veel succes. De klok start — nu!"

## 🧑‍🏫 Tijdens het spel

- [ ] Houd de tijd in de gaten (de HUD-balk bovenin toont de resterende tijd)
- [ ] Loopt een team vast? Wijs op de hint-knop vóórdat je zelf helpt
- [ ] Echt muurvast? Admin portaal → „Sla over" voor dat level
- [ ] Blokkade opheffen kan via admin portaal → „Wis alle blokkades"

## 🏁 Na afloop

- [ ] Noteer de eindtijd (staat op de finale-pagina) voor een eventueel klassement
- [ ] Admin portaal → **Reset alles** voor de volgende groep
- [ ] (Alleen fysieke variant) Verstop het level 4-briefje opnieuw
